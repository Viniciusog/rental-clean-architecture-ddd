...
public interface JpaRentalRepository extends JpaRepository<RentalEntity, Long> {
    ...
    @Query(value = "SELECT COUNT(*) > 0 FROM (SELECT 1 FROM Rental r " +
            "WHERE r.car_id = :carId AND r.id <> :rentalId AND NOT (r.end_time < :startTime OR r.start_time > :endTime) LIMIT 1) AS subquery",
            nativeQuery = true)
    boolean existsByCarIdAndTimeRangeWithRentalExclusion(@Param("carId") Long carId,
                                                         @Param("startTime") Instant startTime,
                                                         @Param("endTime") Instant endTime,
